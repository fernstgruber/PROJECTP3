...
res50m_mitsaga <- c("slope_DTM_50m_avg_ws3","Slope","longc_DTM_50m_avg_ws11","slope_DTM_50m_avg_ws7","crosc_DTM_50m_avg_ws7",
...
predictors <- c("crosc_DTM_50m_avg_ws7")
oldprednames<- c("crosc_DTM_50m_avg_ws7_50m")
dependent=dependentlist[3]
### get raster names##########################################################################
...
if(length(predictors)>1){
  for (i in predictors[2:length(predictors)]){
    temp <- readRAST(paste(i,"@",mapsetnew,sep=""))@data
    data[[i]] <- temp[[i]]
  }
}
...
pdf(paste("./data/fits/plots/",outname,".pdf",sep=""))
...
############################################################################################################################################################
####SET PREDICTORS AND DEPENDENT#############################################################
predictors <- c("Slope_hr")
oldprednames<- c("Slope_hr")
dependent=dependentlist[3]
### get raster names##########################################################################
ST = "SUEDTIROL_DTM_NEU"                                                                     #
ST_mapsets= list.dirs(paste(gisDbase,"/",ST,sep=""),recursive = F,full.names = F)            #
...
############################################################################################################################################################
####SET PREDICTORS AND DEPENDENT#############################################################
predictors <- c("crosc_DTM_50m_avg_ws7","slope_DTM_50m_avg_ws7")
oldprednames<- c("crosc_DTM_50m_avg_ws7_50m","slope_DTM_50m_avg_ws7_50m")
dependent=dependentlist[3]
### get raster names##########################################################################
ST = "SUEDTIROL_DTM_NEU"                                                                     #
ST_mapsets= list.dirs(paste(gisDbase,"/",ST,sep=""),recursive = F,full.names = F)            #
...
############################################################################################################################################################
####SET PREDICTORS AND DEPENDENT#############################################################
predictors <- c("crosc_DTM_50m_avg_ws7","slope_DTM_50m_avg_ws7", "Convexity_50m")
oldprednames<- c("crosc_DTM_50m_avg_ws7_50m","slope_DTM_50m_avg_ws7_50m","Convexity_50m")
dependent=dependentlist[3]
### get raster names##########################################################################
ST = "SUEDTIROL_DTM_NEU"                                                                     #
ST_mapsets= list.dirs(paste(gisDbase,"/",ST,sep=""),recursive = F,full.names = F)            #
