...
require(rgrass7)
dependentnr=1
predictors = c("geom_dtm_10m_hyd_fl5_L10","minic_ws9_hr","CrossSectionalCurvature_hr","profc_ws19_hr","maxic_ws11","slope_DTM_50m_avg_ws3","Slope",
"General_Curvature_hr","slope_ws29_hr","geom_10m_fl1_L110","slope_DTM_50m_avg_ws7","geom_10m_fl3_L3","Longitudinal_Curvature_hr",
"Convergence_Index_hr","slope_ws5_hr","dtm_hr_CONVEX_r20","geom_10m_fl4_L10","slope_ws3","longc_DTM_50m_avg_ws9","crosc_ws5","crosc_ws29_hr",
"geom_10m_fl4_L9","minic_ws3","slope_DTM_50m_avg_ws9","profc_ws9_hr","longc_ws11")

oldprednames=c("geom_dtm_10m_hyd_fl5_L10","minic_ws9_hr_hr","CrossSectionalCurvature_hr","profc_ws19_hr_hr","maxic_ws11_10m","slope_DTM_50m_avg_ws3_50m","Slope_50m",
"General_Curvature_hr","lope_ws29_hr_hr","geom_10m_fl1_L110","slope_DTM_50m_avg_ws7_50m","geom_10m_fl3_L3","Longitudinal_Curvature_hr",
"Convergence_Index_hr","slope_ws5_hr_hr","dtm_hr_CONVEX_r20","geom_10m_fl4_L10","slope_ws3_10m","longc_DTM_50m_avg_ws9_50m","crosc_ws5_10m","crosc_ws29_hr_hr",
"geom_10m_fl4_L9","minic_ws3_10m","slope_DTM_50m_avg_ws9_50m","profc_ws9_hr_hr","longc_ws11_10m")
...
goodpreds <- predictors[predictors %in% allpredictors]
badpreds <- predictors[!(predictors %in% allpredictors)]
goodoldpreds <- oldprednames[predictors %in% allpredictors]
goodpreds
badpreds
predictors %in% oldprednames
######################################################################################
##############################################################################################################
##############################################################################################################
##############################################################################################################
...
predictors <- goodpreds
oldprednames <- goodoldpreds
### get raster names##########################################################################
...
mapsetnew=paste("top50_",dependent,sep="")
...
save(fit,CM,hist,modeldata,preddata,file=paste("/mnt/bola/rebo/5_Daten/Fabian_ausmisten/fits/","top15","_",dependent,".RData",sep=""))
#load(file=paste("/mnt/bola/rebo/5_Daten/Fabian_ausmisten/fits/","top15","_",dependent,".RData",sep=""))
preddata[["preds"]] <- as.integer(preddata[["preds"]])
SPDF <- pred1
tempdata <- data[c("UID")]
temppreddata <- preddata[c("UID","preds")]
print("merging predictions and full data set (containing NA's)")
SPDFdata <- merge(tempdata,temppreddata,by="UID",all.x=T)
print("ordering dataset")
SPDFdata <-SPDFdata[order(SPDFdata$UID,decreasing = F),]
SPDF@data <- SPDFdata
summary(SPDF)
outname=paste("Top15_",dependent,sep="")
